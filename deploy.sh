...
ssh -i ssh_root_key root@$1 "systemctl stop anki-robot.target && mount -o rw,remount / && mkdir -p /anki/data/cloudswitch/ && mkdir -p /anki/data/cloudswitch/sherpa"
...
scp -i ssh_root_key extra/setfreq root@$1:/anki/bin/
ssh -i ssh_root_key root@$1 "sed -i \"s/Nice=\-2/Nice=3/g\" /usr/lib/systemd/system/vic-anim.service"
rsync -e 'ssh -i ssh_root_key' -avr build/en-US root@$1:/anki/data/cloudswitch/
...
if ssh -i ssh_root_key root@$1 "[[ ! -d /anki/data/cloudswitch/sherpa/citrinet-256-ls ]]"; then
	rsync -e 'ssh -i ssh_root_key' -avr build/sherpa-shared-citrinet/citrinet-256-ls root@$1:/anki/data/cloudswitch/sherpa/
fi
ssh -i ssh_root_key root@$1 "systemctl daemon-reload && chmod +rwx /anki/bin/setfreq && sudo -k && systemctl start anki-robot.target"
